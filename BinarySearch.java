...
class BinarySearch implements SearchAlgorithm {
    ...
    private <T extends Comparable<T>> int search(
        ...
        if(right < left) {
            return -1; // that means that the key not found
        }
    ...
    public static void main(String[] args) {
        ...
        Integer[] integers = IntStream.generate(() -> r.nextInt(maxElement))
            .limit(size).sorted().boxed().toArray(Integer[]::new);
        ...
        System.out.println(format("should be found: %d. Found %d at index %d. An array length %d", shouldBeFound, integers[atIndex], atIndex, size));
